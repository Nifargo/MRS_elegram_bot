...
# Усі способи написати діапазон зводяться до одного дефіса ще до перевірки —
# інакше кожен новий різновид тире («900‒1400») чи слова («від 900 до 1400»)
# треба було б додавати окремою гілкою, і рівно один із них колись пропустили б.
_DASH_LIKE = str.maketrans({c: "-" for c in "‒–—―−/"})
_RANGE_WORD_RE = re.compile(r"(\d)\s+до\s+(\d)", re.IGNORECASE)
...
def amounts_in(text: str) -> set[int]:
    ...
    Діапазон дає обидві межі, як би його не написали: «900–1400 грн»,
    «900/1400 грн», «від 900 до 1400 грн».
    ...
    """
    normalized = _RANGE_WORD_RE.sub(r"\1-\2", text.translate(_DASH_LIKE))
    found = set()
