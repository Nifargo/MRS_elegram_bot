...
# Усі способи перелічити дві суми під одну валюту зводяться до одного дефіса ще
# до перевірки — інакше кожен різновид тире («900‒1400») і кожен сполучник
# («900 та 1400 грн») був би окремою гілкою, і рівно один із них колись
# пропустили б. Перша сума в такому переліку і є те місце, де модель може
# назвати ціну, якої в прайсі немає.
_DASH_LIKE = str.maketrans({c: "-" for c in "‒–—―−/"})
_JOINER_RE = re.compile(r"(\d)\s+(?:до|та|і|й|або)\s+(\d)", re.IGNORECASE)

# Межі діапазону — тільки цифри й пробіли (без крапок і ком), а перед лівою
# межею не може стояти «+», цифра, крапка чи кома. Інакше нормалізоване
# «+380960080482 - 1300 грн» чи «20.08 - 1300 грн» давало б «суму» з телефона
# або дати, а хибні спрацювання їдять бюджет сповіщень адмінам.
_RANGE_NUM = r"\d[\d\s\u00a0]*"
...
_AMOUNT_RE = re.compile(
    rf"(?:"
    rf"(?<![+\d.,])({_RANGE_NUM}?)\s*-\s*({_RANGE_NUM})\s*{_CURRENCY}"
    rf"|({_NUM})\s*{_CURRENCY}"
    rf"|₴\s*({_NUM})"
    rf")",
    re.IGNORECASE,
...
def amounts_in(text: str) -> set[int]:
    ...
    Дві суми під одну валюту дають обидві, як би їх не з'єднали: «900–1400 грн»,
    «900/1400 грн», «від 900 до 1400 грн», «900 та 1400 грн».
    ...
    Межа покриття: число перевіряється, якщо межує з валютою або з'єднане з
    таким числом сполучником напряму. Поза перевіркою лишаються число, відділене
    від валюти іменником («стрижка 300 та комплекс 1300 грн»), перелік із трьох
    позицій під одну валюту, суми словами і ліва межа діапазону, приклеєна до
    коми чи цифри без пробілу («Комплекс,900-1400 грн»). Розширити це можна лише
    ціною хибних спрацювань на вагах і тривалостях, яких у блоці даних повно, а
    вони б'ють по єдиному сигналу адмінам про зламаний запобіжник. Дешевше
    працює інший бік: SYSTEM_PROMPT велить моделі писати «грн» біля кожної суми.
    """
    normalized = _JOINER_RE.sub(r"\1-\2", text.translate(_DASH_LIKE))
    found = set()
