...
#
# Extended with Java/Gradle/Maven support.
...
# --- Java / Gradle ---
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+test(\s|$)'; then
  SUGGESTION="rtk ./gradlew test"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+compileJava(\s|$)'; then
  SUGGESTION="rtk ./gradlew compileJava"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+compileTestJava(\s|$)'; then
  SUGGESTION="rtk ./gradlew compileTestJava"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+build(\s|$)'; then
  SUGGESTION="rtk ./gradlew build"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+clean(\s|$)'; then
  SUGGESTION="rtk ./gradlew clean"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+check(\s|$)'; then
  SUGGESTION="rtk ./gradlew check"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+checkstyle'; then
  SUGGESTION=$(echo "$CMD" | sed 's/^\./rtk ./')
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+dependencies(\s|$)'; then
  SUGGESTION="rtk ./gradlew dependencies"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+tasks(\s|$)'; then
  SUGGESTION="rtk ./gradlew tasks"
elif echo "$FIRST_CMD" | grep -qE '^\./gradlew\s+'; then
  # Catch-all for any other gradlew command
  SUGGESTION=$(echo "$CMD" | sed 's/^\./rtk ./')
...
# --- Maven ---
elif echo "$FIRST_CMD" | grep -qE '^\./mvnw\s+'; then
  SUGGESTION=$(echo "$CMD" | sed 's/^\./rtk ./')
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+test(\s|$)'; then
  SUGGESTION="rtk mvn test"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+compile(\s|$)'; then
  SUGGESTION="rtk mvn compile"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+package(\s|$)'; then
  SUGGESTION="rtk mvn package"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+clean(\s|$)'; then
  SUGGESTION="rtk mvn clean"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+install(\s|$)'; then
  SUGGESTION="rtk mvn install"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+verify(\s|$)'; then
  SUGGESTION="rtk mvn verify"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+dependency:tree(\s|$)'; then
  SUGGESTION="rtk mvn dependency:tree"
elif echo "$FIRST_CMD" | grep -qE '^mvn\s+'; then
  SUGGESTION=$(echo "$CMD" | sed 's/^mvn /rtk mvn /')
