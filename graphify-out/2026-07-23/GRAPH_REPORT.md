...
Cohesion: 0.05
Nodes (61): create_client_record(), create_notification(), create_pet(), delete_pet(), get_client_by_id(), get_client_by_phone(), get_client_by_tg_id(), get_last_past_tracked_record() (+53 more)
...
Cohesion: 0.14
Nodes (43): parse_weight(), float, int, Розібрати вагу в кг (кома або крапка). None, якщо не число або поза межами 0.1–1, Викликати Telegram-запит (reply_text/reply_location/...) з повторами.      Прокс, with_retry(), _ask_location, _ask_pet_photo (+35 more)

### Community 3 - "Підтвердження запису та створення в Altegio"
Cohesion: 0.11
Nodes (44): _ask_category, _ask_time, _category_keyboard, _confirm_booking, _show_location_card, Знайти або створити Altegio-клієнта для обраної локації.      clients.altegio_co, resolve_altegio_client_id(), _company_ids_preferred_first (+36 more)

### Community 4 - "Утиліти дати/часу"
Cohesion: 0.15
Nodes (29): datetime, parse_iso_datetime(), str, Спільні валідатори полів анкети та карток улюбленців., Захисний парсинг timestamptz-рядка з Supabase/Altegio (інколи із 'Z' замість off, 2026-08-01', '10:00' -> aware ISO-рядок у Europe/Kyiv, для запису в timestamptz-, to_kyiv_iso(), _ask_reschedule_date() (+21 more)
...
## Knowledge Gaps
- **72 isolated node(s):** `graphify-remind.sh script`, `allow`, `deny`, `defaultMode`, `enabled` (+67 more)
...
- **Are the 19 inferred relationships involving `AltegioError` (e.g. with `bool` and `str`) actually correct?**
  _`AltegioError` has 19 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `InlineKeyboardButton` (e.g. with `show_bookings()` and `_show_reschedule_date_page()`) actually correct?**
  _`InlineKeyboardButton` has 10 INFERRED edges - model-reasoned connections that need verification._
- **What connects `graphify-remind.sh script`, `allow`, `deny` to the rest of the system?**
