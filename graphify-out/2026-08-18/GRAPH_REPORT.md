# Graph Report - grooming-telegram-bot  (2026-08-18)

## Corpus Check
- 72 files · ~40,117 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1032 nodes · 2386 edges · 73 communities (41 shown, 32 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 120 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `697e24ac`
...
5. `show_menu_button()` - 30 edges
6. `process_record()` - 27 edges
7. `Update` - 25 edges
8. `int` - 25 edges
9. `DEFAULT_TYPE` - 24 edges
10. `handle_callback()` - 24 edges
...
Cohesion: 0.09
Nodes (73): _ask_service(), book_start() — «📅 Записатись», _confirm_booking(), format_price(), generic_breed_services(), location_name(), match_services_by_breed(), price_start() — «💰 Дізнатись вартість» (+65 more)

### Community 1 - "Visit History and Shared Validators"
Cohesion: 0.05
Nodes (62): parse_date(), date, get_clients_with_altegio_link(), Клієнти, прив'язані до Altegio (для щоденної синхронізації вакцинації, Фаза 10)., parse_date(), bool, date, Розібрати дату у форматі ДД.ММ.РРРР. None, якщо формат/значення некоректні. (+54 more)
...
Cohesion: 0.06
Nodes (54): Application, main() (polling entrypoint), CRON_SECRET, TELEGRAM_TOKEN, main(), Запуск бота (polling, для локальної розробки)., check_webhook(), get_webhook_info() (+46 more)

### Community 4 - "Slot Selection and Record Upsert"
Cohesion: 0.07
Nodes (59): resolve_altegio_client_id(), create_client_record(), Створити порожній запис клієнта (реєстрацію заповнюємо покроково)., booking._ask_date, booking._ask_time, booking._confirm_booking, Знайти або створити Altegio-клієнта для обраної локації.      clients.altegio_co, my_bookings._ask_reschedule_date (+51 more)

### Community 5 - "Webhook Reconciliation and Cancellation"
Cohesion: 0.14
Nodes (23): normalize_phone(), get_client_by_phone(), Створити або оновити кеш запису (fields повинні містити altegio_record_id)., Знайти клієнта за телефоном (формат +380XXXXXXXXX). Для матчингу Altegio-вебхукі, upsert_tracked_record(), normalize_phone(), Привести телефон до формату +380XXXXXXXXX. None, якщо номер не схожий на українс, Migration 007: visit notifications (ends_at + altegio_record_id FK) (+15 more)

### Community 6 - "Client Registration Flow"
Cohesion: 0.09
Nodes (69): with_retry(), Оновити поля клієнта., update_client(), DEFAULT_TYPE, hide_menu_button(), int, Приховати нативну кнопку «Меню» (/start, /cancel) на час активного флоу     (анк, Викликати Telegram-запит (reply_text/reply_location/...) з повторами.      Прокс (+61 more)
...
Cohesion: 0.15
Nodes (30): parse_weight(), parse_weight(), float, Розібрати вагу в кг (кома або крапка). None, якщо не число або поза межами 0.1–1, _card_keyboard(), _card_text(), _delete_confirm_keyboard(), edit_cancel() (+22 more)
...
Cohesion: 0.11
Nodes (27): HELP_PHONE, SYSTEM_PROMPT, clear_chat_history(), get_response(), Отримати відповідь від Groq для повідомлення користувача., Очистити історію чату для користувача., chat_histories (in-memory per-user dict), clear_chat_history() (+19 more)
...
Cohesion: 0.13
Nodes (22): Dual retry policy: interactive (flat, fast) vs background (exponential, patient), Надіслати повідомлення напряму через Bot API (з ретраями на транзиентні 502/503/, send_telegram_message(), _clear_booking_state(), get_due_notifications(), _handle_booking_incomplete(), _handle_form_incomplete(), _handle_rebook_nudge() (+14 more)

### Community 17 - "Admin Alerts and Error Handling"
Cohesion: 0.22
Nodes (10): Позначити статус запису (напр. cancelled при скасуванні в Altegio)., update_tracked_record_status(), my_bookings._do_cancel, bool, Пройтись по всіх філіях і звірити записи на найближчі RECONCILE_WINDOW_DAYS днів, reconcile_upcoming_records(), _handle (webhook payload dispatch), Single idempotent record-ingest path for webhook and reconcile (+2 more)

### Community 18 - "Free-Slot Rebook Promo"
Cohesion: 0.10
Nodes (20): get_active_tracked_records_with_ends_at(), get_pets_with_birth_date(), Активні записи з відомим ends_at (перевірка прострочених rebook, Фаза 7).      Б, Усі улюбленці з відомою датою народження (щоденна перевірка днів народження, Фаз, bool, Привітати клієнтів, чий улюбленець народився сьогодні (без урахування року)., send_birthday_greetings(), last_promo_at cooldown as anti-spam pacing (+12 more)
...
Cohesion: 0.25
Nodes (9): bool, bytes, has_pending_notification(), has_tracked_record_since(), bool, Чи з'явився активний запис клієнта після вказаного часу (перевірка, чи флоу запи, Чи є вже заплановане (pending) сповіщення цього типу для клієнта., Надіслати файл напряму через Bot API. Повертає True при успіху.      `content` — (+1 more)
...
Cohesion: 0.09
Nodes (24): ADMIN_GROUP_CHAT_ID, ADMIN_TOPIC_ID, GOOGLE_MAPS_REVIEW_URLS, handle_error(), DEFAULT_TYPE, Глобальний обробник помилок PTB.  Без нього виняток, що вилетів з будь-якого han, handle_callback(), _owns_record() (+16 more)
...
Cohesion: 0.13
Nodes (18): create_notification(), delete_pending_notifications_for_record(), get_active_tracked_records_in_range(), get_cron_last_run(), mark_notification(), str, Активні записи філії в межах вікна дат (для щоденної звірки: скасовані в Altegio, Запланувати сповіщення (send_after — ISO timestamp). (+10 more)

### Community 64 - "Community 64"
Cohesion: 0.10
Nodes (32): create_pet(), create_rating(), delete_pet(), get_client_by_id(), get_client_by_tg_id(), get_pet(), get_pets_by_client(), get_rating() (+24 more)

### Community 65 - "Community 65"
Cohesion: 0.25
Nodes (6): datetime, Щоденна звірка записів Altegio з локальним кешем tracked_records.  Вебхуки (serv, Фаза 7, п.1: день народження улюбленця — щоденне привітання (без знижки, поки що, Створення та відправка сповіщень.  Відправка йде напряму через Telegram Bot API, (Пере)планувати reminder_2h/thanks_rating/rebook_nudge для запису.      Ідемпоте, schedule_visit_notifications()

### Community 67 - "Community 67"
Cohesion: 0.36
Nodes (6): BuildDumpTest, build_dump(): усі таблиці в дампі, великі таблиці читаються сторінками., Ланка .range() у ланцюжку supabase.table().select().order().range()., test_dump_contains_every_table_and_counts(), test_reads_all_pages_when_table_exceeds_page_size(), test_sorts_by_declared_column()
...
## Knowledge Gaps
- **190 isolated node(s):** `bytes`, `SendDocumentTest`, `Global Constraints`, `File Structure`, `code:python ("""send_telegram_document(): файл іде в sendDocument як mult)` (+185 more)
...
- **Why does `AltegioError` connect `Visit History and Shared Validators` to `Booking and Price Flow`, `Slot Selection and Record Upsert`, `Client Registration Flow`, `Breed-Based Service Matching`, `Rebook Promo Tests`, `Free-Slot Rebook Promo`, `Promo Callback Ownership Checks`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `with_retry()` connect `Client Registration Flow` to `Booking and Price Flow`, `Visit History and Shared Validators`, `Breed-Based Service Matching`, `Pet Card Management`, `Notification Scheduler Dispatch`, `Promo Callback Ownership Checks`, `Telegram Retry Tests`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `datetime` connect `Community 65` to `Community 64`, `Booking and Price Flow`, `Visit History and Shared Validators`, `Webhook Reconciliation and Cancellation`, `Breed-Based Service Matching`, `Pet Card Management`, `Rebook Promo Tests`, `Notification Scheduler Dispatch`, `Free-Slot Rebook Promo`, `Promo Callback Ownership Checks`, `Telegram Retry Tests`?**
