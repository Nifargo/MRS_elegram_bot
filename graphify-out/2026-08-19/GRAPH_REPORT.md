# Graph Report - grooming-telegram-bot  (2026-08-19)

## Corpus Check
- 82 files · ~54,349 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1253 nodes · 2625 edges · 94 communities (60 shown, 34 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 139 edges (avg confidence: 0.73)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d24e7a8b`
...
Cohesion: 0.06
Nodes (117): _ask_service(), book_start() — «📅 Записатись», _confirm_booking(), format_price(), generic_breed_services(), location_name(), match_services_by_breed(), price_start() — «💰 Дізнатись вартість» (+109 more)

### Community 1 - "Visit History and Shared Validators"
Cohesion: 0.06
Nodes (55): get_active_tracked_records_with_ends_at(), Активні записи з відомим ends_at (перевірка прострочених rebook, Фаза 7).      Б, bool, _format_visit(), handle_callback(), DEFAULT_TYPE, int, str (+47 more)
...
Cohesion: 0.07
Nodes (62): resolve_altegio_client_id(), create_client_record(), Створити або оновити кеш запису (fields повинні містити altegio_record_id)., Створити порожній запис клієнта (реєстрацію заповнюємо покроково)., upsert_tracked_record(), booking._ask_date, booking._ask_time, booking._confirm_booking (+54 more)

### Community 5 - "Webhook Reconciliation and Cancellation"
Cohesion: 0.19
Nodes (18): get_client_by_phone(), Знайти клієнта за телефоном (формат +380XXXXXXXXX). Для матчингу Altegio-вебхукі, Webhook Reconciliation of Bookings Made Outside the Bot, _handle (webhook payload dispatch), _format_dt(), _handle(), _location_name(), _parse_kyiv() (+10 more)

### Community 6 - "Client Registration Flow"
Cohesion: 0.11
Nodes (53): normalize_phone(), Оновити поля клієнта., update_client(), normalize_phone(), Привести телефон до формату +380XXXXXXXXX. None, якщо номер не схожий на українс, registration._ask_location, registration._ensure_altegio_link, registration._search_altegio_client (+45 more)
...
Cohesion: 0.08
Nodes (49): parse_date(), parse_weight(), get_clients_with_altegio_link(), Клієнти, прив'язані до Altegio (для щоденної синхронізації вакцинації, Фаза 10)., parse_date(), parse_weight(), bool, date (+41 more)
...
Cohesion: 0.09
Nodes (28): DEFAULT_TYPE, Exception, _owning_client (callback ownership check), handle_callback(), IDOR protection for record_id in callback_data, _owning_client(), DEFAULT_TYPE, int (+20 more)
...
Cohesion: 0.17
Nodes (14): date, build_dump(), _fetch_table(), is_backup_due(), str, Тижневий бекап Supabase: повний дамп таблиць одним JSON-файлом у Telegram.  На б, Зібрати дамп і надіслати файлом власнику. True — Telegram підтвердив доставку., Усі рядки таблиці, сторінками не більше PAGE_SIZE.      Кожна наступна сторінка (+6 more)
...
Cohesion: 0.24
Nodes (10): get_client_by_id(), get_client_by_tg_id(), Знайти клієнта за Telegram user_id. Повертає None, якщо не знайдено., Знайти клієнта за внутрішнім id., my_bookings._get_own_record, pets._get_own_pet, rating._owns_record, IDOR protection: verify callback-data-referenced resource belongs to requesting tg_user_id (+2 more)
...
Cohesion: 0.12
Nodes (23): has_tracked_record_since(), Чи з'явився активний запис клієнта після вказаного часу (перевірка, чи флоу запи, clients.rebook_promo_dismissed_record_id column, _clear_booking_state(), get_due_notifications(), _handle_booking_incomplete(), _handle_form_incomplete(), _handle_rebook_nudge() (+15 more)
...
Cohesion: 0.13
Nodes (23): create_pet(), delete_pet(), get_last_past_tracked_record(), get_pet(), get_pets_by_client(), get_rating(), get_tracked_record(), get_tracked_record_by_id() (+15 more)
...
Cohesion: 0.14
Nodes (12): BuildDumpTest, Тижневий бекап: склад дампа, сторінкове читання, гейтування по тижню, відправка., Ланцюжок supabase.table().select().order().limit()[.gt()] одним моком., Гілка в _run_daily_tasks(): бекап викликається і позначається датою суботи., SchedulerWiringTest, SendWeeklyBackupTest, test_created_at_is_iso_with_kyiv_offset(), test_dump_contains_every_table_and_counts() (+4 more)
...
Cohesion: 0.16
Nodes (16): bool, handle_error(), DEFAULT_TYPE, Глобальний обробник помилок PTB.  Без нього виняток, що вилетів з будь-якого han, object, Dual retry policy: interactive (flat, fast) vs background (exponential, patient), notify_admins(), notify_admins_async() (+8 more)
...
Cohesion: 0.18
Nodes (12): bytes, datetime, int, Створення та відправка сповіщень.  Відправка йде напряму через Telegram Bot API, Запланувати перевірку 'клієнт не заповнив анкету' на сьогодні 21:00 (Київ)., (Пере)планувати reminder_2h/thanks_rating/rebook_nudge для запису.      Ідемпоте, (Пере)планувати reminder_2h/thanks_rating/rebook_nudge для запису.      Ідемпоте, Надіслати файл напряму через Bot API. Повертає True при успіху.      `content` — (+4 more)
...
Cohesion: 0.11
Nodes (18): AI-консультант на живих даних салону + захист від ін'єкцій, Видимість спрацювань, Вичерпана квота Groq, Вміст контексту, Головний інваріант: у промпт не потрапляє текст, написаний клієнтом, Деградація, Детерміновані перевірки відповіді, Заодно виправляємо (+10 more)
...
Cohesion: 0.29
Nodes (7): _stub_backup(), test_attempt_is_marked_before_run_with_today(), test_backup_exception_does_not_mark_week_done(), test_decision_gets_kyiv_now_and_both_marks(), test_failed_backup_does_not_mark_week_done(), test_not_due_skips_backup(), test_saturday_run_marks_week_done()

### Community 92 - "Community 92"
Cohesion: 0.15
Nodes (13): create_notification(), delete_pending_notifications_for_record(), has_pending_notification(), bool, Запланувати сповіщення (send_after — ISO timestamp)., Видалити pending-сповіщення заданих типів для конкретного запису (Фаза 4: remind, Чи є вже заплановане (pending) сповіщення цього типу для клієнта., booking._select_service (+5 more)

### Community 93 - "Community 93"
Cohesion: 0.25
Nodes (9): create_rating(), Записати оцінку послуги/грумера для завершеного візиту., handle_callback(), _owns_record(), bool, DEFAULT_TYPE, int, Update (+1 more)
...
- **Why does `AltegioError` connect `Visit History and Shared Validators` to `Booking and Price Flow`, `Slot Selection and Record Upsert`, `Client Registration Flow`, `Breed-Based Service Matching`, `Pet Card Management`, `Rebook Promo Tests`, `Admin Alerts and Error Handling`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `datetime` connect `Welcome Message` to `Community 64`, `Booking and Price Flow`, `Visit History and Shared Validators`, `Community 67`, `Webhook Reconciliation and Cancellation`, `Community 69`, `Breed-Based Service Matching`, `Pet Card Management`, `Rebook Promo Tests`, `Community 81`, `Admin Alerts and Error Handling`, `Community 86`, `Promo Callback Ownership Checks`, `Community 91`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `with_retry()` connect `Booking and Price Flow` to `Visit History and Shared Validators`, `Client Registration Flow`, `Breed-Based Service Matching`, `Pet Card Management`, `Community 71`, `Rebook Promo Tests`, `Community 93`?**
